"""
Implements HRL framework with options to solve the taxi env: 

1. Option Class:
   - Represents an option designed to navigate to a specific landmark goal.
   - Goal locations are mapped from the environment's landmark coordinates:
     R: (0, 0) - Top-Left
     G: (0, 4) - Top-Right
     Y: (4, 0) - Bottom-Left
     B: (4, 3) - Bottom-Right
   - Each option trains its own internal Q-network (a simple multi-layer perceptron)
     that learns to output action values based solely on the taxi's (row, col) position.
   - Training uses standard Q-learning with epsilon-greedy exploration.
   - beta: The termination condition for the option, which returns True when the
     taxi reaches the option's designated goal location.
   - pi: The option's policy, selecting the action with the highest Q-value.
   - Epsilon decay parameters, discounts, and location name dictionaries are defined globally.

2. Agent Class:
   - The top-level agent that decides between primitive actions (move south, north, 
     east, west, pickup, dropoff) and option policies.
   - Uses Semi-Markov Decision Process (SMDP) Q-learning for temporal abstraction, 
     updating state-action values over multi-step option executions.
   - Uses Intra-Option Q-learning updates, which allow options to learn from 
     experiences generated by the behavior policy of other options or primitive actions.
   - greedy: Selects the action/option with the maximum Q-value.
   - choose_action: Epsilon-greedy action selection.

3. Evaluation and Plotting:
   - evaluate_agent: Evaluates the hierarchical agent over multiple episodes, computes 
     performance statistics, and generates evaluation plots.
   - plot_episode_timelines: Creates a horizontal timeline bar chart showing action and 
     option usage across all timesteps for all evaluation episodes.
   - plot_episode_detail: Plots the exact movement path and actions/options executed by 
     the agent during a single evaluation episode.
   - plot_option_q_policies: Visualizes the greedy policy of each option's internal 
     Q-network over the 5x5 grid using arrows and heatmaps.
   - plot_episodes_grid: Generates a 3x4 grid visualization showing detailed paths for the
     first 12 evaluation episodes.
   - _draw_episode_on_ax: Helper function to draw an episode's path onto a matplotlib axis.
...
def plot_option_q_policies(agent, env, output_path='option_q_policies.png'):
    ...
    for idx, opt in enumerate(agent.options):
        ...
        cmap = plt.cm.colors.ListedColormap(
            [action_colors[i] for i in range(4)]
        )
        ax.imshow(policy_grid, cmap=cmap, vmin=0, vmax=3,
